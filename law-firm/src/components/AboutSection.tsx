...
export const AboutSection = () => {
  ...
  return (
    <section id="about" className="py-28 bg-black relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none" style={{ backgroundImage: "radial-gradient(rgba(201,168,76,0.04) 1px, transparent 1px)", backgroundSize: "40px 40px" }} />
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          {/* Left — Text */}
          <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.9, ease: "easeOut" }}>
            <div className="relative">
              <motion.div className="absolute -top-4 -left-4 w-16 h-16 border-t border-l border-primary/40" initial={{ opacity: 0, scale: 0 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: 0.4, duration: 0.5 }} />
              <motion.div className="absolute -bottom-4 -right-4 w-16 h-16 border-b border-r border-primary/40" initial={{ opacity: 0, scale: 0 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: 0.5, duration: 0.5 }} />
          ...
          {/* Right — Visual */}
          <motion.div initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.9, delay: 0.2, ease: "easeOut" }} className="hidden lg:block relative h-full min-h-[600px]">
            <div className="absolute inset-0 bg-white/[0.015] border border-white/8 rounded-xl overflow-hidden flex items-center justify-center backdrop-blur-sm">
              ...
              <div className="absolute inset-0" style={{ backgroundImage: "radial-gradient(rgba(201,168,76,0.05) 1px, transparent 1px)", backgroundSize: "28px 28px" }} />
              {[300, 240, 180, 120, 64].map((size, i) => (
                <motion.div key={i} className="absolute rounded-full"
                  style={{ width: size, height: size, border: `${i === 4 ? "2px" : "1px"} ${i % 2 === 0 ? "dashed" : "solid"} rgba(201,168,76,${0.06 + i * 0.06})` }}
                  animate={{ rotate: i % 2 === 0 ? 360 : -360 }} transition={{ duration: 15 + i * 10, repeat: Infinity, ease: "linear" }} />
              ))}
              ...
              <div className="relative z-10 flex flex-col items-center gap-6">
                <motion.div className="w-36 h-36 rounded-full flex items-center justify-center"
                  style={{ background: "radial-gradient(circle, rgba(201,168,76,0.18) 0%, transparent 70%)", border: "1.5px solid rgba(201,168,76,0.4)", boxShadow: "0 0 60px rgba(201,168,76,0.2)" }}
                  animate={{ boxShadow: ["0 0 40px rgba(201,168,76,0.15)", "0 0 80px rgba(201,168,76,0.3)", "0 0 40px rgba(201,168,76,0.15)"] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}>
                  <img src={ganeshLogo} alt="Logo" className="w-24 h-24 object-contain" style={{ filter: "sepia(1) saturate(4) hue-rotate(5deg) brightness(1.15) drop-shadow(0 0 20px rgba(201,168,76,0.8))" }} />
                </motion.div>
                <div className="text-center space-y-2">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-[9px] text-white/40 tracking-[0.4em] uppercase">Est. 2000</div>
                    <div className="flex-1 h-[1px] bg-primary/30" />
                  </div>
